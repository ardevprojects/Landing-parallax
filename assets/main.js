...
const pages = document.querySelectorAll('.hide');
...
const track = document.getElementById('image-row');
...
window.onmousemove = e => {
  ...
  const percent = (delta / maxDelta) * -100,
    nextPercentOpen = parseFloat(track.dataset.prevPercent) + percent,
    nextPercent = Math.max(Math.min(nextPercentOpen, 0), -100);
  track.dataset.percent = nextPercent;

  track.animate({
    transform: `translateX(${nextPercent}%)`
...
const contactButton = document.querySelector('#btn-nav-contact');
const box = document.querySelector('.conditional');
const inbox = document.querySelector('.contact-container');

const contactButtonClick = () => {
  box.classList.add('contact-blur');
  box.classList.remove('off');
};

const offClick = () => {
  box.classList.remove('contact-blur');
...
document.addEventListener('click', function (event) {
  const outsideClick = !contactButton.contains(event.target) && !inbox.contains(event.target);
  outsideClick && offClick();
});


const navMediaChange = () => {
  let home = document.getElementById('nav-home');
  let explore = document.getElementById('nav-explore');
  let map = document.getElementById('nav-map');
  let contact = document.getElementById('btn-nav-contact');
  ...
  if (window.innerWidth < 600) {
    ...
    explore.innerHTML =
      '<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="#FFFFFF"><path d="M0 0h24v24H0z" fill="none"/><path d="M11.99 18.54l-7.37-5.73L3 14.07l9 7 9-7-1.63-1.27-7.38 5.74zM12 16l7.36-5.73L21 9l-9-7-9 7 1.63 1.27L12 16z"/></svg>';
    map.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="#FFFFFF"><path d="M0 0h24v24H0z" fill="none"/><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>';
    contact.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="#10051a"><path d="M0 0h24v24H0zm0 0h24v24H0zm0 0h24v24H0z" fill="none"/><path d="M20 0H4v2h16V0zM4 24h16v-2H4v2zM20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm-8 2.75c1.24 0 2.25 1.01 2.25 2.25s-1.01 2.25-2.25 2.25S9.75 10.24 9.75 9 10.76 6.75 12 6.75zM17 17H7v-1.5c0-1.67 3.33-2.5 5-2.5s5 .83 5 2.5V17z"/></svg>'
  } else {
    home.innerHTML = 'Home'
...
onresize = (event) => { navMediaChange() };

document.querySelector('.main-container').classList.add('container', 'added', 'min')